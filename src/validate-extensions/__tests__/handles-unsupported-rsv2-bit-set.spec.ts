 ...
 *
 * RSV2 is reserved and must be 0 unless negotiated by an extension.
 *
 * This test verifies that when RSV2 is set in the frame header, but no
 * extension claims support for it, an error is thrown as required by the RFC.
 */
describe('validateExtensions', () => {
  it('rfc-6455-5.2 rsv2=1 unsupported', () => {
    ...
    expect(() => validateExtensions(header, extensionSupport)).toThrow(
      'RSV2 is set but not supported by any extension',
    );
  });
});
